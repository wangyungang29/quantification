...
class ChanStrategy(bt.Strategy):
    ...
    def next(self):
        ...
        if current_signal and current_signal.get('is_valid', False):
            if current_signal['type'] in ['一买', '二买', '三买'] and self.position.size == 0:
                ...
                if trade_size > 0:
                    ...
                    signal_base_low = current_signal.get('base_low', 0)
                    price_based_stop = current_price * (1 - self.p.stop_loss_ratio)
                    if signal_base_low > 0:
                        self.stop_loss = min(price_based_stop, signal_base_low * 1.02)
                    else:
                        self.stop_loss = price_based_stop
                    ...
                    print(f"买入信号({current_signal['type']}): {current_date}, 价格={current_price:.2f}, 数量={trade_size}, 止损位={self.stop_loss:.2f}")

    def _update_chan_signals(self):
        """更新缠论信号"""
        data = self.datas[0]
        size = min(len(self), 200)
